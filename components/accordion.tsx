   ...
   const AccordionItem = ({ isOpen, onClick }) => {
    const contentHeight = useRef()
     ...
     return(
       ...
       <button className={` w-full text-left py-3  px-1  flex items-center justify-between font-medium  border-none pointer leading-none xs:py-2  ${isOpen ? '' : ''}`} onClick={onClick} >
  

        <p className=' text-[28px] font-semibold xs:text-base'>Members benefits</p>
        <img src={'/assets/icons/caret-down.svg'} alt='' className={`w-3 ease-out duration-300    ${isOpen ? 'rotate-[180deg]' : ''}`}/>

       </button>
    
        <div ref={contentHeight} className="  ease-out duration-300 " style={
             isOpen
